...
public class RoutingSlipManager
{
    ...
    public RoutingSlipBuilder ScenarioOne() =>
        RoutingSlipBuilder.Default
            .AddActivity<WithdrawActivity>("sample", new WithdrawArgument("ACC-101", 200))
            .AddActivity<DepositActivity>("sample", new DepositArgument("ACC-101", 200));

    public RoutingSlipBuilder ScenarioPortfolio() =>
        RoutingSlipBuilder.Default
            .AddActivity<DebitActivity>("debit1", new DebitCommand("ACC1", 100))
            .AddActivity<DebitActivity>("debit2", new DebitCommand("ACC1", 950))
            .AddActivity<CreditActivity>("credit1", new CreditCommand("ACC2", 1050))
            .AddVariables(new { compMode = 1 });

    public RoutingSlipBuilder ScenarioPortfolioExceptional() =>
        RoutingSlipBuilder.Default
            .AddActivity<DebitActivity>("debit1", new DebitCommand("ACC1", 100))
            .AddActivity<DebitActivity>("debit2", new DebitCommand("ACC1", 1050))
            .AddActivity<CreditActivity>("credit1", new CreditCommand("ACC2", 1150))
            .AddVariables(new { compMode = 1 });
